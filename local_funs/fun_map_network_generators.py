...
def fun_map_network_generators(carrier, n, feature, file_regions, path_regions, ax, params, params_local):
    """
    ...
    gdf_network_generators = fun_gdf_network_generators(carrier, n, file_regions, path_regions)



    ##### Unwrap params
    if not params_local['vmin']:
        params_local['vmin'] = gdf_network_generators[feature].min()

    if not params_local['vmax']:
        params_local['vmax'] = gdf_network_generators[feature].max()



    ##### Plot in map

    gdf_network_generators.plot(ax=ax, column=feature, 
